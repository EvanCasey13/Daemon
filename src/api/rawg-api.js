export const fetchPopular = (page, query, genres) => {
    return fetch(
        `https://rawg.io/api/games?token&key=${process.env.REACT_APP_RAWG_API_KEY}&page=${page}&genres=${genres}&page_size=18&search=${query}`
    ).then((response) => {
      if (!response.ok) {
